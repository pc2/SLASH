...
namespace vrt {
    ...
    uint64_t Allocator::allocate(uint64_t size, MemoryRangeType type) {
        ...
        if (size < superblockSize / 2) {
            ...
            while (addr + size <= range.startAddress + range.size) {
                ...
                bool isOccupied = std::any_of(range.usedMemoryBlocks.begin(), range.usedMemoryBlocks.end(), [addr, size](const std::pair<uint64_t, uint64_t>& block) {
                    return (addr >= block.first && addr < block.first + block.second) || (addr + size > block.first && addr + size <= block.first + block.second);
                });
    ...
    uint64_t  Allocator::allocate(uint64_t size, MemoryRangeType type, uint8_t port) {
        ...
        uint64_t portEndAddress = portBaseAddress + 2 * HBM_PORT_SIZE * 8; // allow moving to next port (aligned at 64bit)

        if (size < superblockSize / 2) {
            for (auto& superblock : range.superblocks) {
                if(superblock.startAddress < portBaseAddress || superblock.startAddress >= portBaseAddress + HBM_PORT_SIZE) {
                    continue;
                }
                try {
                    uint64_t addr = superblock.allocate(size);
            ...
            bool isOccupied = std::any_of(range.usedMemoryBlocks.begin(), range.usedMemoryBlocks.end(), [portBaseAddress, sbs](const std::pair<uint64_t, uint64_t>& block) {
                return (portBaseAddress >= block.first && portBaseAddress < block.first + block.second) || (portBaseAddress + sbs > block.first && portBaseAddress + sbs <= block.first + block.second);
            });
            ...
            if (!isOccupied) {
                ...
                for (const auto& block : range.usedMemoryBlocks) {
                    if (nextFreeAddress >= block.first && nextFreeAddress < block.first + block.second) {
            ...
            if (!range.freeList.empty()) {
                auto it = std::find_if(range.freeList.begin(), range.freeList.end(), [portBaseAddress](uint64_t addr) {
                        return addr > portBaseAddress;
                    });
            ...
            //     if (superblock.startAddress >= portBaseAddress && superblock.startAddress < portEndAddress) {
            ...
            // }
            for (const auto& block : range.usedMemoryBlocks) {
                if (block.first + block.second >= portBaseAddress && block.first + block.second < portEndAddress) { // block.first >= portBaseAddress && block.first < portEndAddress
                    //if (nextFreeAddress >= block.first && nextFreeAddress < block.first + block.second) {
                        nextFreeAddress = block.first + block.second;
                    //}
                }
            ...
            if (nextFreeAddress + size <= portEndAddress) {
                range.usedMemoryBlocks.push_back({nextFreeAddress, size}); // Keep track of the used block
